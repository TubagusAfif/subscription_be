...
export class MegaBankTokenUtil {
  ...
  public static async getAccessToken(): Promise<string> {
    if (this.cachedToken && this.cachedToken.expiresAt - TOKEN_REFRESH_BUFFER_MS > Date.now()) {
  ...
  private static async fetchAccessToken(): Promise<string> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logger.error('[MegaBankTokenUtil] Access token request failed', {
          status: response.status,
          body: errorBody,
        });
        throw new AppError(
          'MEGA_BANK_AUTH_ERROR',
          `Access token request failed (HTTP ${response.status}): ${errorBody}`,
          502,
        );
      }
      ...
      if (!data.accessToken) {
        throw new AppError(
          'MEGA_BANK_AUTH_ERROR',
          `Access token response missing accessToken: ${data.responseMessage}`,
          502,
        );
      }
      ...
      if (isNaN(expiresInSeconds) || expiresInSeconds <= 0) {
        logger.warn('[MegaBankTokenUtil] Invalid expiresIn, using default TTL', {
          raw: data.expiresIn,
          defaultSeconds: DEFAULT_TOKEN_TTL_SECONDS,
        });
      }
      const safeTtlSeconds =
        isNaN(expiresInSeconds) || expiresInSeconds <= 0
          ? DEFAULT_TOKEN_TTL_SECONDS
          : expiresInSeconds;

      const expiresInMs = safeTtlSeconds * 1000;
      this.cachedToken = { accessToken: data.accessToken, expiresAt: Date.now() + expiresInMs };
      logger.info('[MegaBankTokenUtil] Access token acquired', {
        logic: 'Token received successfully, caching for (expiresIn) seconds',
