...
import { ClientAccountRepository } from '../../../../src/client/repositories/account.repository';
...
describe('ClientAccountService', () => {
  let accountService: ClientAccountService;
  let mockAccountRepository: jest.Mocked<ClientAccountRepository>;
  ...
  beforeEach(() => {
    ...
    } as unknown as jest.Mocked<ClientAccountRepository>;

    accountService = new ClientAccountService(mockAccountRepository);
  });
  ...
  describe('changePassword', () => {
    ...
    it('should throw VALIDATION_ERROR if new password is the same as current password', async () => {
      mockAccountRepository.findByIdWithProfile.mockResolvedValue(mockUserWithProfile as any);
      (bcrypt.compare as jest.Mock).mockResolvedValue(true);
