...
import { createAccountRouter } from '../../../../src/shared/routes/account.routes';
import { AccountController } from '../../../../src/shared/controllers/account.controller';
...
describe('Account Routes Integration', () => {
  ...
  beforeEach(() => {
    ...
    const accountController = new AccountController({ accountService: mockAccountService });
    ...
    const router = createAccountRouter(accountController, mockAuthenticate as any);

    app = express();
    app.use(express.json());
    app.use('/api/v1/shared/account', router);
    app.use(errorHandler);
  });

  describe('GET /api/v1/shared/account/me', () => {
  ...
  describe('PATCH /api/v1/shared/account/password', () => {
    it('should return 400 if currentPassword missing', async () => {
      const response = await request(app)
        .patch('/api/v1/shared/account/password')
