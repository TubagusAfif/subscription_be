...
import type { SubscribeBody, SwitchPlanBody } from '../../shared/validations/subscription.validation';
...
export class ClientSubscriptionController {
  ...
  subscribe = async (req: AuthenticatedRequest<SubscribeBody>, res: Response, next: NextFunction): Promise<void> => {
    try {
      const { sku_id } = req.body;
      const subscription = await this.subscriptionService.subscribe(Number(req.user.sub), sku_id);
      res.status(201).json(
        successResponse(SubscriptionMapper.toResponse(subscription)),
      );
    } catch (error) {
      next(error);
    }
  ...
  getMySubscription = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
  ...
  getMySubscriptions = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
      const subscriptions = await this.subscriptionService.getMySubscriptions(Number(req.user.sub));
      res.status(200).json(
        successResponse(subscriptions.map((s) => SubscriptionMapper.toResponse(s))),
      );
    } catch (error) {
      next(error);
    }
  ...
  switchPlan = async (req: AuthenticatedRequest<SwitchPlanBody>, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      res.status(200).json(
        successResponse(SubscriptionMapper.toResponse(newSubscription)),
      );
    } catch (error) {
      next(error);
    }
