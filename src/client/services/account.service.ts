  ...
  async updateAccount(userId: number, data: UpdateAccountDTO): Promise<UserWithProfile> {
    ...
    if (data.profile) {
      ...
      if (data.profile.date_of_birth !== undefined) {
        profileData.date_of_birth = data.profile.date_of_birth
          ? new Date(data.profile.date_of_birth)
          : null;
      }
      if (data.profile.gender !== undefined) profileData.gender = data.profile.gender;
      if (data.profile.address_line1 !== undefined)
        profileData.address_line1 = data.profile.address_line1;
      if (data.profile.address_line2 !== undefined)
        profileData.address_line2 = data.profile.address_line2;
      if (data.profile.city !== undefined) profileData.city = data.profile.city;
      if (data.profile.utc_timezone !== undefined)
        profileData.utc_timezone = data.profile.utc_timezone;
      if (data.profile.province !== undefined) profileData.province = data.profile.province;
      if (data.profile.postal_code !== undefined)
        profileData.postal_code = data.profile.postal_code;
      if (data.profile.country !== undefined) profileData.country = data.profile.country;
      if (data.profile.clinic_name !== undefined)
        profileData.clinic_name = data.profile.clinic_name;
