...
export class CurrencyController {
  ...
  createCurrency = async (
    req: AuthenticatedRequest<CreateCurrencyBody>,
    res: Response,
    next: NextFunction,
  ): Promise<void> => {
    try {
  ...
  getAllCurrencies = async (
    ...
    try {
      ...
      res.status(200).json(
        successResponse(
          data.map((c) => CoinMapper.toCurrencyResponse(c)),
          meta,
        ),
      );
    } catch (error) {
  ...
  updateCurrency = async (
    req: AuthenticatedRequest<UpdateCurrencyBody>,
    res: Response,
    next: NextFunction,
  ): Promise<void> => {
    try {
  ...
  activateCurrency = async (
    req: AuthenticatedRequest,
    res: Response,
    next: NextFunction,
  ): Promise<void> => {
    try {
      const currency = await this.currencyService.activateCurrency(
        Number(req.params.id),
        Number(req.user.sub),
      );
