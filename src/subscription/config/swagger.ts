...
const spec = {
  ...
  info: {
    ...
    version: '1.0.0',
    description:
      'Full REST API for the clinic subscription platform. Covers client auth, coin wallet, subscriptions, clinic management, operational users and the subscription-admin domain.',
  },
  servers: [
    {
      url: '/api/v1',
  ...
  components: {
    ...
    schemas: {
      ...
      CreatePlanRequest: {
        type: 'object',
        required: ['sku_name', 'sku_code', 'sku_type', 'coin_cost'],
        properties: {
          id: {
            type: 'integer',
            nullable: true,
          ...
          package_tier: {
            type: 'string',
            enum: ['BASIC', 'LITE', 'MEDIUM', 'PRO', 'ENTERPRISE'],
            nullable: true,
            example: 'PRO',
          },
          rank: {
      ...
      CreateCurrencyRequest: {
        type: 'object',
        required: ['currency_name', 'currency_code', 'symbol', 'conversion_rate', 'effective_from'],
        properties: {
          currency_name: {
            type: 'string',
      ...
      CreateBundleRequest: {
        type: 'object',
        required: ['bundle_name', 'coin_amount', 'currency_id'],
        properties: {
          bundle_name: {
            type: 'string',
      ...
      CreateTaxRequest: {
        type: 'object',
        required: ['tax_name', 'tax_value', 'tax_type'],
        properties: {
          tax_name: {
            type: 'string',
            example: 'PPN',
  ...
  paths: {
    ...
    '/subscription/currencies': {
      post: {
        tags: ['Currencies'],
        summary:
          'Create a coin currency (auto-deactivates all other currencies when is_active is true)',
        description:
          'Creates a new currency. When is_active is true (default), all existing active currencies are automatically deactivated to enforce a single active currency policy.',
        requestBody: {
          required: true,
          content: {
            'application/json': {
    ...
    '/subscription/currencies/active': {
      get: {
        ...
        description:
          'Retrieves the single active currency currently enforced across the subscription domain.',
        responses: {
          '200': {
    ...
    '/subscription/currencies/{id}/activate': {
      patch: {
        ...
        description:
          'Sets the specified currency as the only active currency. All other currencies are atomically deactivated within a transaction.',
        parameters: [
          {
            in: 'path',
            name: 'id',
    ...
    '/subscription/taxes': {
      post: {
        ...
        description:
          'Creates a new tax configuration. When is_active is true (default), all existing active taxes are automatically deactivated to enforce a single active tax policy.',
        requestBody: {
          required: true,
          content: {
            'application/json': {
    ...
    '/subscription/taxes/active': {
      get: {
        ...
        description:
          'Retrieves the single active tax configuration currently enforced across the subscription domain.',
        responses: {
          '200': {
    ...
    '/subscription/taxes/{id}': {
      ...
      put: {
        ...
        description:
          'Updates a tax configuration. When is_active is set to true, all other active taxes are automatically deactivated to enforce a single active tax policy.',
        parameters: [
          {
            in: 'path',
            name: 'id',
    ...
    '/subscription/taxes/{id}/activate': {
      patch: {
        ...
        description:
          'Sets the specified tax as the only active tax. All other taxes are atomically deactivated within a transaction.',
        parameters: [
          {
            in: 'path',
            name: 'id',
    ...
    '/shared/webhook/mpg': {
      post: {
        ...
        security: [],
        description:
          'Called by Mega Payment Gateway after a payment event. Verifies webhook signature and updates order/wallet status.',
        requestBody: {
          required: true,
          content: {
            'application/json': {
